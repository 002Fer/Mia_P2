...
func Login(user string, pass string, id string) {
	...
	if index != -1 {
		Structs.PrintPartition(TempMBR.M_Partitions[index])
	} else {
	...
	// Read object from bin file
	if err := Utilities.Leer_Object(file, &tempSuperblock, int64(TempMBR.M_Partitions[index].Start)); err != nil {
		return
	}
	...
	// Read object from bin file
	if err := Utilities.Leer_Object(file, &crrInode, int64(tempSuperblock.S_inode_start+indexInode*int32(binary.Size(Structs.Inode{})))); err != nil {
		return
	}

...
func InitSearch(path string, file *os.File, tempSuperblock Structs.Superblock) int32 {
	...
	if err := Utilities.Leer_Object(file, &Inode0, int64(tempSuperblock.S_inode_start)); err != nil {
		return -1
	}
	...
	for _, block := range Inode.I_block {
		if block != -1 {
			if index < 13 {
				...
				// Read object from bin file
				if err := Utilities.Leer_Object(file, &crrFolderBlock, int64(tempSuperblock.S_block_start+block*int32(binary.Size(Structs.Folderblock{})))); err != nil {
					return -1
				}
				...
				for _, folder := range crrFolderBlock.B_content {
					...
					if strings.Contains(string(folder.B_name[:]), SearchedName) {
						...
						if len(StepsPath) == 0 {
							...
							// Read object from bin file
							if err := Utilities.Leer_Object(file, &NextInode, int64(tempSuperblock.S_inode_start+folder.B_inodo*int32(binary.Size(Structs.Inode{})))); err != nil {
								return -1
							}
	...
	for _, block := range Inode.I_block {
		if block != -1 {
			if index < 13 {
				...
				// Read object from bin file
				if err := Utilities.Leer_Object(file, &crrFileBlock, int64(tempSuperblock.S_block_start+block*int32(binary.Size(Structs.Fileblock{})))); err != nil {
					return ""
				}
